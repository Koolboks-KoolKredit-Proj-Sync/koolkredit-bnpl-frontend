...
// ─── Mono API helpers ────────────────────────────────────────────────────────
const MONO_SECRET_KEY = 'live_sk_z2zqd4k2ayqv9z9t0dqi';
const MONO_PUBLIC_KEY = 'live_pk_f1sfmbiu5olyu958z5ne';
const MONO_API_BASE   = 'https://api.withmono.com';

async function monoClient(endpoint, body) {
  const config = {
  ...
  const res = await fetch(`${MONO_API_BASE}/${endpoint}`, config);
  if (!res.ok) throw new Error(`Mono API error: ${res.status}`);
  return res.json();
}

async function monoAuthenticate(code) {
  return monoClient('account/auth', { code });
}

async function monoGetAccount(id) {
  return monoClient(`accounts/${id}`);
}

async function monoGetCredits(id) {
  return monoClient(`accounts/${id}/credits`);
}

async function monoGetDebits(id) {
  return monoClient(`accounts/${id}/debits`);
}
// ─────────────────────────────────────────────────────────────────────────────
...
function AgentEntryForm() {
  ...
  // ── Mono widget state ──────────────────────────────────────────────────────
  const [monoScriptLoaded, setMonoScriptLoaded] = useState(false);
  const [monoScriptError, setMonoScriptError] = useState(false);
  const [isFetchingMono, setIsFetchingMono] = useState(false);
  const [monoFinancialData, setMonoFinancialData] = useState(null); // { account, credits, debits }
  const monoInstanceRef = useRef(null);

  // Load Mono script once on mount
  useEffect(() => {
    if (document.getElementById('mono-connect-script')) {
      setMonoScriptLoaded(true);
      return;
    }
    ...
    script.onerror = () => {
      setMonoScriptError(true);
      console.error('Failed to load Mono Connect script');
    };
    document.body.appendChild(script);
  }, []);

  // Re-init widget whenever script loads
  useEffect(() => {
    if (monoScriptLoaded) initMonoWidget();
  }, [monoScriptLoaded]);

  function initMonoWidget() {
    if (typeof window.Connect === 'undefined') return;

    const options = {
      onSuccess: async (response) => {
        setIsFetchingMono(true);
        try {
          // 1. Exchange code for account ID
          const authData = await monoAuthenticate(response.code);
          const accountId = authData.id;

          // 2. Fetch account, credits and debits in parallel
          const [accountRes, creditsRes, debitsRes] = await Promise.all([
            monoGetAccount(accountId),
            monoGetCredits(accountId),
            monoGetDebits(accountId),
          ]);

          const financialData = {
            accountId,
            account: accountRes.account || accountRes,
            credits: creditsRes.history || creditsRes.data || [],
            debits:  debitsRes.history  || debitsRes.data  || [],
            creditTotal: creditsRes.total || 0,
            debitTotal:  debitsRes.total  || 0,
          };

          setMonoFinancialData(financialData);

          Swal.fire({
            icon: 'success',
            title: 'Bank Statement Retrieved',
            text: 'Financial data fetched successfully from your bank account.',
          });
        } catch (err) {
          console.error('Mono fetch error:', err);
          Swal.fire({
            icon: 'error',
            title: 'Mono Error',
            text: 'Failed to fetch financial data. Please try again.',
          });
        } finally {
          setIsFetchingMono(false);
          if (monoInstanceRef.current) monoInstanceRef.current.close();
        }
      },
      onClose: () => {
        // widget closed by user — no-op
      },
    };

    const connectMono = new window.Connect(MONO_PUBLIC_KEY, options);
    connectMono.setup();
    monoInstanceRef.current = connectMono;
  }

  // Handler for both Mono and MBS buttons
  const handleBankStatementMethod = (method) => {
    setFormData(prev => ({ ...prev, bankStatementMethod: method }));

    if (monoScriptError) {
      Swal.fire({
        icon: 'error',
        title: 'Script Error',
        text: 'Mono Connect script failed to load. Please refresh the page.',
      });
      return;
    }
    ...
    if (!monoScriptLoaded) {
      Swal.fire({
        icon: 'warning',
        title: 'Loading...',
        text: 'Mono Connect is still loading. Please try again in a moment.',
      });
      return;
    }

    // Re-init in case it wasn't ready earlier
    if (!monoInstanceRef.current) initMonoWidget();
    ...
    if (monoInstanceRef.current) {
      ...
      Swal.fire({
        ...
        text: 'Unable to launch bank connection. Please refresh and try again.',
      });
    }
  };
  // ──────────────────────────────────────────────────────────────────────────
  ...
  const handleSubmit = async () => {
    ...
    // Include Mono financial data if available
    if (monoFinancialData) {
      formDataToSend.append('monoFinancialData', JSON.stringify(monoFinancialData));
    }
        ...
        if (result.verified) {
          Swal.fire({
            ...
            const completeData = {
              ...
              bankName: preFilledData.bankName,
              accountNumber: preFilledData.accountNumber,
              // ── Pass Mono financial data to follow-up form ──
              monoFinancialData: monoFinancialData || null,
              bankStatementMethod: formData.bankStatementMethod,
            };
            sessionStorage.setItem("customerData", JSON.stringify(completeData));
            window.agentEntryData = completeData;
            navigate('/agent-followup', { state: completeData });
                ...
                <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                          <div>
                            ...
                            <input type="text" name="spouseName" value={formData.spouseName} onChange={handleInputChange}
                                   className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                                   onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                                   placeholder="Enter spouse name" />
                          </div>
                          <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: '#f7623b' }}>Spouse Phone</label>
                            <input type="tel" name="spousePhone" value={formData.spousePhone} onChange={handleInputChange}
                                   className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                                   onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                                   placeholder="Enter spouse phone number" />
                          </div>
                  ...
                  <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-2" style={{ color: '#f7623b' }}>Next of Kin Phone</label>
                        <input type="tel" name="nextOfKinPhone" value={formData.nextOfKinPhone} onChange={handleInputChange}
                               className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                               onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                               placeholder="Enter next of kin phone" />
                      </div>
                  ...
                  <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                      <div>
                        <label className="block text-sm font-medium mb-2" style={{ color: '#f7623b' }}>BVN *</label>
                        <input type="text" name="bvn" value={formData.bvn} onChange={handleInputChange} maxLength="11"
                               className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                               onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                               placeholder="Enter 11-digit BVN" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-2" style={{ color: '#f7623b' }}>NIN *</label>
                        <input type="text" name="nin" value={formData.nin} onChange={handleInputChange} maxLength="11"
                               className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                               onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                               placeholder="Enter 11-digit NIN" />
                      </div>
                  ...
                  {formData.plan === 'Omolope' && (
                      <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                        <h2 className="text-xl sm:text-2xl font-semibold mb-3 sm:mb-4 text-white">Duration (Days) *</h2>
                        <input type="number" name="omolopeDays" value={formData.omolopeDays} onChange={handleInputChange} min="1"
                               className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none transition"
                               onFocus={(e) => e.target.style.borderColor = '#f7623b'} onBlur={(e) => e.target.style.borderColor = '#374151'}
                               placeholder="Enter number of days" />
                      </div>
                  ...
                  {/* ── Bank Statement Section ─────────────────────────────────── */}
                  <div className="pb-4 sm:pb-6">
                    <h2 className="text-xl sm:text-2xl font-semibold mb-1 text-white">Request Bank Statement</h2>
                    <p className="text-gray-400 text-sm mb-4">
                      Click either button to securely connect the customer's bank account via Mono.
                    </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      ...
                      <button
                          type="button"
                          onClick={() => handleBankStatementMethod('Mono')}
                          disabled={isFetchingMono}
                          className="flex items-center justify-center px-4 sm:px-6 py-3 sm:py-4 rounded-lg text-sm sm:text-base font-medium transition cursor-pointer hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                          style={{
                            backgroundColor: formData.bankStatementMethod === 'Mono' ? '#f7623b' : '#111827',
                            color: formData.bankStatementMethod === 'Mono' ? 'white' : '#9ca3af',
                            border: formData.bankStatementMethod === 'Mono' ? 'none' : '1px solid #374151'
                      ...
                      >
                        {isFetchingMono && formData.bankStatementMethod === 'Mono' ? (
                            <><Loader2 className="animate-spin mr-2" size={18} /> Fetching...</>
                        ) : (
                            <><FileText className="mr-2" size={20} /> Mono</>
                        )}
                      </button>

                      {/* MBS Button — same Mono flow */}
                      <button
                          type="button"
                          onClick={() => handleBankStatementMethod('MBS')}
                          disabled={isFetchingMono}
                          className="flex items-center justify-center px-6 py-4 rounded-lg font-medium transition cursor-pointer hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                          style={{
                            backgroundColor: formData.bankStatementMethod === 'MBS' ? '#f7623b' : '#111827',
                            color: formData.bankStatementMethod === 'MBS' ? 'white' : '#9ca3af',
                            border: formData.bankStatementMethod === 'MBS' ? 'none' : '1px solid #374151'
                      ...
                      >
                        {isFetchingMono && formData.bankStatementMethod === 'MBS' ? (
                            <><Loader2 className="animate-spin mr-2" size={18} /> Fetching...</>
                        ) : (
                            <><Upload className="mr-2" size={20} /> MBS</>
                        )}
                      </button>
                    </div>

                    {/* Success badge when Mono data is fetched */}
                    {monoFinancialData && (
                        <div className="mt-4 p-4 bg-green-900/20 border border-green-500/50 rounded-lg">
                          <div className="flex items-center mb-2">
                            <CheckCircle size={16} className="text-green-400 mr-2" />
                            <span className="text-green-400 text-sm font-semibold">Bank Statement Retrieved Successfully</span>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs mt-2">
                            <div className="bg-gray-900 rounded p-2">
                              <p className="text-gray-400">Account Balance</p>
                              <p className="text-white font-bold">
                                ₦{((monoFinancialData.account?.balance || 0) / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}
                              </p>
                            </div>
                            <div className="bg-gray-900 rounded p-2">
                              <p className="text-gray-400">Credit Transactions</p>
                              <p className="text-white font-bold">{monoFinancialData.credits?.length || 0} records</p>
                            </div>
                            <div className="bg-gray-900 rounded p-2">
                              <p className="text-gray-400">Debit Transactions</p>
                              <p className="text-white font-bold">{monoFinancialData.debits?.length || 0} records</p>
                            </div>
                          </div>
                          <p className="text-gray-500 text-xs mt-2">
                            Full financial report will be emailed to admin after follow-up form submission.
                          </p>
                        </div>
                    )}
                  </div>
                ...
                <div className="relative mb-3 sm:mb-4">
                  <div className="relative overflow-hidden rounded-lg bg-gray-800">
                    ...
                    <video ref={videoRef} autoPlay playsInline muted className="w-full rounded-lg"
                           style={{ transform: facingMode === 'user' ? 'scaleX(-1)' : 'scaleX(1)', minHeight: '300px', maxHeight: '500px', objectFit: 'cover' }} />
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="border-4 rounded-full bg-transparent"
                           style={{ width: '280px', height: '350px', borderColor: '#f7623b', boxShadow: '0 0 0 9999px rgba(0, 0, 0, 0.5)' }} />
                    </div>
                    <button onClick={flipCamera}
                            className="absolute top-4 right-4 p-3 rounded-full text-white shadow-lg transition hover:opacity-80 cursor-pointer"
                            style={{ backgroundColor: '#f7623b' }} title="Flip camera">
                      <RefreshCw size={20} />
                    </button>
                  </div>
                  <p className="text-center text-gray-400 text-sm mt-2">
