...
//     const [guarantorEmail, setGuarantorEmail] = useState("");
//
//     const BACKEND_API_URL = 'https://web-production-88f7c.up.railway.app';
//
//     const handleUtilityUpload = (e) => {
//         const file = e.target.files[0];
//         if (file) {
...
//             // Submit the agent follow-up form
//             const response = await fetch(
//                 "https://web-production-88f7c.up.railway.app/api/agent-followup",
//                 { method: "POST", body: formData }
//             );
//
//             if (!response.ok) {
...
export default function AgentFollowUpForm() {
    ...
    const BACKEND_API_URL = 'https://web-production-88f7c.up.railway.app';
    const CREDIT_PULL_URL = 'https://koolkredit-payment-integration-production-2c72.up.railway.app/v1/credit-enquiries/pull/';
    ...
    const updateDebitMandateCustomerDetails = async () => {
        if (!entryData?.mandateReference) return;
        try {
            const payload = {
                bvn: entryData.bvn,
                customerAddress: homeAddress,
    ...
    const handleSubmit = async (e) => {
        ...
        if (!entryData?.bvn) return Swal.fire("Error", "BVN is required", "error");
        if (!utilityBillFile) return Swal.fire("Error", "Please upload a utility bill", "error");
        if (usage === "Personal" && !monthlyIncome) return Swal.fire("Error", "Monthly Income is required", "error");
        if (usage === "Commercial" && !monthlySales) return Swal.fire("Error", "Monthly Sales is required", "error");
        if (!entryData?.firstName || !entryData?.lastName) {
            return Swal.fire("Error", "Customer name information is missing. Please go back and complete the previous form.", "error");
        }
            ...
            Swal.fire({
                ...
                html: `
                    <p style="color:#6b7280; font-size:14px; margin-top:8px;">
                        Your application has been received and is currently under review by our team.<br/><br/>
                        You will receive an OTP via SMS once the review is complete.
                    </p>
                `,
                confirmButtonColor: '#f7623b',
        ...
        <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8" style={{ backgroundColor: '#f7623b' }}>
            <div className="w-full max-w-4xl bg-black rounded-2xl shadow-2xl p-6 sm:p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                    {/* Verification Details */}
                    <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                        <h2 className="text-xl sm:text-2xl font-semibold mb-4 text-white">Verification Details</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">BVN *</label>
                                <input type="text" value={entryData?.bvn || ""} readOnly
                                       className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">NIN</label>
                                <input type="text" value={entryData?.nin || ""} readOnly
                                       className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">BVN Mobile Number *</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">Your Email *</label>
                                <input type="email" placeholder="youremail@something.com" value={customerEmail}
                            ...
                            {entryData?.passportPreview && (
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Passport Photo</label>
                                    <img src={entryData.passportPreview} alt="Passport"
                                         className="h-32 w-32 object-cover rounded-lg border border-[#f7623b]" />
                                </div>
                            )}
                        </div>
                    ...
                    <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">Plan</label>
                                <input type="text" value={entryData?.plan || ""} readOnly
                                       className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-2 text-[#f7623b]">Installment Option</label>
                                <input type="text" value={entryData?.installmentDuration || ""} readOnly
                                       className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                            </div>
                        </div>
                    </div>
                    ...
                    {usage === "Personal" && (
                        <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Home Address *</label>
                                    <input type="text" placeholder="Enter Home Address" value={homeAddress}
                                           onChange={(e) => setHomeAddress(e.target.value)} required
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Work Address</label>
                                    <input type="text" placeholder="Enter Work Address" value={workAddress}
                                           onChange={(e) => setWorkAddress(e.target.value)}
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Monthly Gross Income *</label>
                                    <input type="number" placeholder="Enter Monthly Gross Income" value={monthlyIncome}
                                           onChange={(e) => setMonthlyIncome(e.target.value)} required min="0" step="0.01"
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Guarantor Email *</label>
                                    <input type="email" placeholder="guarantor@example.com" value={guarantorEmail}
                                           onChange={(e) => setGuarantorEmail(e.target.value)} required
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                            </div>
                        </div>
                    ...
                    {usage === "Commercial" && (
                        <div className="border-b pb-4 sm:pb-6" style={{ borderColor: '#f7623b' }}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Store Address *</label>
                                    <input type="text" placeholder="Enter Store Address" value={storeAddress}
                                           onChange={(e) => setStoreAddress(e.target.value)} required
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Monthly Sales *</label>
                                    <input type="number" placeholder="Enter Monthly Sales" value={monthlySales}
                                           onChange={(e) => setMonthlySales(e.target.value)} required min="0" step="0.01"
                                           className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#f7623b] transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-[#f7623b]">Guarantor Email *</label>
                    ...
                    <div className="pb-4 sm:pb-6">
                        ...
                        <div className="flex items-center gap-3">
                            <label className="p-3 bg-gray-900 border border-gray-700 rounded-lg cursor-pointer flex items-center gap-2 text-white hover:border-[#f7623b] transition">
                                <Upload size={18} /> Upload
                                <input type="file" accept="image/*" onChange={handleUtilityUpload} className="hidden" />
                            </label>
                            <label className="p-3 bg-gray-900 border border-gray-700 rounded-lg cursor-pointer flex items-center gap-2 text-white hover:border-[#f7623b] transition">
                                <Camera size={18} /> Snap
                                <input type="file" accept="image/*" capture="environment" onChange={handleUtilityUpload} className="hidden" />
                            </label>
                        </div>
                        {utilityBill && (
                            <img src={utilityBill} alt="Utility Bill" className="h-40 mt-3 rounded-lg border border-[#f7623b] object-cover" />
                        )}
                    </div>
