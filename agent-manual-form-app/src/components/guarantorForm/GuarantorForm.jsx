...
const Stage2SVG = () => (
    <svg viewBox="0 0 200 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* OR divider and head turn arrows */}
    ...
    useEffect(() => {
        ...
        detectionIntervalRef.current = setInterval(async () => {
            ...
            try {
                ...
                // ── STAGE 2: Smile OR head turn left/right ───────────
                if (stage === 2) {
                    const mouth = landmarks.getMouth();
                    const nose = landmarks.getNose();

                    // Mouth aspect ratio — smile opens mouth wide
                    ...
                    if (isSmiling) {
                        setProgress(p => ({ ...p, smiled: true }));
                    }
                    if (isTurned) {
                        setProgress(p => ({ ...p, turned: true }));
                    }
                    ...
                    <div className="text-xs text-gray-400 text-center">
                        ...
                        {stage === 2 && (
                            <span className={progress.smiled || progress.turned ? "text-green-400" : ""}>
                                {progress.smiled
                                    ? "😊 Smile detected — keep going..."
                                    : progress.turned
                                        ? "↔️ Head turn detected — keep going..."
...
const SelfieLiveness = ({ onComplete }) => {
    ...
    const stages = [
        {
            title: "Liveness Check — Blink",
            description: "Look at the camera and blink your eyes once.",
            svg: <Stage1SVG />,
            color: "from-orange-900 to-black",
        },
        {
            title: "Liveness Check — Smile or Turn",
            description: "Give a smile 😊 or gently turn your head left or right — either works!",
            svg: <Stage2SVG />,
            color: "from-purple-900 to-black",
        },
        {
