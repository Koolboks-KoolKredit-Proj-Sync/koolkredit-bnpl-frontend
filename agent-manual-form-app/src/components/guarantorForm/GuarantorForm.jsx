...
const Stage1SVG = () => (
    <svg viewBox="0 0 200 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <g>
            ...
            <circle cx="82" cy="84" r="5" fill="#3b2a1a"/>
            <circle cx="84" cy="82" r="2" fill="white"/>
            <animate attributeName="opacity" values="1;1;0.1;1;1" dur="2.5s" repeatCount="indefinite"/>
        </g>
        <g>
            <ellipse cx="118" cy="82" rx="9" ry="9" fill="white"/>
            <circle cx="118" cy="84" r="5" fill="#3b2a1a"/>
        ...
        <rect x="73" y="73" width="18" height="0" rx="2" fill="#f7c59f">
            <animate attributeName="height" values="0;18;0" dur="2.5s" repeatCount="indefinite"/>
        </rect>
        <rect x="109" y="73" width="18" height="0" rx="2" fill="#f7c59f">
            <animate attributeName="height" values="0;18;0" dur="2.5s" repeatCount="indefinite"/>
        </rect>
        <ellipse cx="100" cy="100" rx="5" ry="7" fill="#e8a87c"/>
        <path d="M 85 115 Q 100 125 115 115" fill="none" stroke="#c0392b" strokeWidth="2.5" strokeLinecap="round"/>
        <path d="M 60 65 Q 68 72 75 78" fill="none" stroke="#f7623b" strokeWidth="2" strokeLinecap="round" markerEnd="url(#blinkArrow1)"/>
        <path d="M 140 65 Q 132 72 125 78" fill="none" stroke="#f7623b" strokeWidth="2" strokeLinecap="round" markerEnd="url(#blinkArrow2)"/>
        <defs>
            <marker id="blinkArrow1" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
                <path d="M0,0 L0,6 L6,3 z" fill="#f7623b"/>
            </marker>
            <marker id="blinkArrow2" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
                <path d="M6,0 L6,6 L0,3 z" fill="#f7623b"/>
            </marker>
        </defs>
        <text x="100" y="158" textAnchor="middle" fill="#f7623b" fontSize="12" fontWeight="bold">Blink Your Eyes</text>
    </svg>
);

const Stage2SVG = () => (
    <svg viewBox="0 0 200 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="100" cy="85" rx="45" ry="52" fill="#f7c59f" stroke="#f7623b" strokeWidth="2.5"/>
        <ellipse cx="100" cy="40" rx="45" ry="18" fill="#3b2a1a"/>
        <ellipse cx="82" cy="78" rx="9" ry="9" fill="white"/>
        <circle cx="82" cy="80" r="5" fill="#3b2a1a"/>
        <circle cx="84" cy="78" r="2" fill="white"/>
        <ellipse cx="118" cy="78" rx="9" ry="9" fill="white"/>
        <circle cx="118" cy="80" r="5" fill="#3b2a1a"/>
        <circle cx="120" cy="78" r="2" fill="white"/>
        <ellipse cx="100" cy="96" rx="5" ry="6" fill="#e8a87c"/>
        <path d="M 76 110 Q 100 138 124 110" fill="#e07070" stroke="#c0392b" strokeWidth="2" strokeLinecap="round">
            <animate attributeName="d" values="M 82 112 Q 100 122 118 112;M 76 110 Q 100 140 124 110;M 82 112 Q 100 122 118 112" dur="2s" repeatCount="indefinite"/>
        </path>
        <ellipse cx="65" cy="105" rx="11" ry="7" fill="#f4a0a0" opacity="0.5"/>
        <ellipse cx="135" cy="105" rx="11" ry="7" fill="#f4a0a0" opacity="0.5"/>
        <text x="100" y="158" textAnchor="middle" fill="#888" fontSize="10" fontWeight="bold">— OR —</text>
        <path d="M 38 85 L 58 85" fill="none" stroke="#f7623b" strokeWidth="2.5" strokeLinecap="round" markerEnd="url(#leftArr)"/>
        <path d="M 162 85 L 142 85" fill="none" stroke="#f7623b" strokeWidth="2.5" strokeLinecap="round" markerEnd="url(#rightArr)"/>
        <defs>
            <marker id="leftArr" markerWidth="6" markerHeight="6" refX="6" refY="3" orient="auto">
                <path d="M0,0 L0,6 L6,3 z" fill="#f7623b"/>
            </marker>
            <marker id="rightArr" markerWidth="6" markerHeight="6" refX="0" refY="3" orient="auto">
                <path d="M6,0 L6,6 L0,3 z" fill="#f7623b"/>
            </marker>
        </defs>
        <text x="100" y="172" textAnchor="middle" fill="#f7623b" fontSize="12" fontWeight="bold">Smile or Turn Head</text>
    </svg>
);
...
const LivenessCamera = ({ stage, onStageComplete }) => {
    ...
    const [faceApiLoaded, setFaceApiLoaded] = useState(false);
    const [loadingMessage, setLoadingMessage] = useState("Starting camera...");
    const [progress, setProgress] = useState({ blinkCount: 0, smiled: false, turned: false });
    const [progressPct, setProgressPct] = useState(0);
    const [capturedImage, setCapturedImage] = useState(null);
    ...
    useEffect(() => {
        ...
        detectionIntervalRef.current = setInterval(async () => {
            ...
            try {
                ...
                const avgEAR = (eyeAR(leftEye) + eyeAR(rightEye)) / 2;
                if (stage === 1) {
                    if (avgEAR < 0.28 && blinkStateRef.current === "open") {
                        blinkStateRef.current = "closed";
                    } else if (avgEAR > 0.23 && blinkStateRef.current === "closed") {
                ...
                if (stage === 2) {
                    ...
                    const eyeWidth = Math.abs(rightEye[3].x - leftEye[0].x);
                    const noseOffset = eyeWidth > 0 ? Math.abs(noseTip.x - eyeCenterX) / eyeWidth : 0;
                    const isSmiling = mouthRatio > 0.28;
                    const isTurned = noseOffset > 0.12;
                    if (isSmiling) setProgress(p => ({ ...p, smiled: true }));
                    if (isTurned) setProgress(p => ({ ...p, turned: true }));
                    if (isSmiling || isTurned) {
                        progressPctRef.current = Math.min(progressPctRef.current + 18, 100);
                        setProgressPct(progressPctRef.current);
                        if (progressPctRef.current >= 100) {
                            clearInterval(detectionIntervalRef.current);
                            setTimeout(() => onStageComplete(), 800);
    ...
    const retake = () => {
        ...
        const startCamera = async () => {
            ...
            } catch (err) { console.error("Retake camera error:", err); }
        };
        startCamera();
    };
    ...
    return (
        <div className="space-y-4">
            ...
            {stage !== 3 && !loadingMessage && (
                <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-400 text-center">
                        {stage === 1 && (
                            <span className={progress.blinkCount >= 1 ? "text-green-400" : ""}>
                                {progress.blinkCount >= 1 ? "✅ Blink detected!" : "👁️ Blink your eyes once"}
                            </span>
                        )}
                        {stage === 2 && (
                            <span className={progress.smiled || progress.turned ? "text-green-400" : ""}>
                                {progress.smiled ? "😊 Smile detected — keep going..."
                                    : progress.turned ? "↔️ Head turn detected — keep going..."
                                        : "😊 Smile at the camera  OR  ↔️ turn your head left / right"}
                            </span>
                        )}
                    </div>
                </div>
            ...
            {stage === 3 && cameraReady && !capturedImage && (
                <button type="button" onClick={captureStage3}
                        className="w-full py-4 bg-[#f7623b] text-white font-bold rounded-xl hover:opacity-90 transition flex items-center justify-center gap-2 text-base">
                    <Camera className="w-5 h-5" /> Take Selfie with ID
                </button>
            )}
            {capturedImage && (
                <div className="flex gap-3">
...
const SelfieLiveness = ({ onComplete }) => {
    ...
    const stages = [
        { title: "Liveness Check — Blink", description: "Look at the camera and blink your eyes once.", svg: <Stage1SVG />, color: "from-orange-900 to-black" },
        { title: "Liveness Check — Smile or Turn", description: "Give a smile 😊 or gently turn your head left or right — either works!", svg: <Stage2SVG />, color: "from-purple-900 to-black" },
        { title: "ID Selfie", description: "Hold your valid Driver's Licence, Voter's Card, NIN slip, or Passport clearly in front of you and take a selfie.", svg: <Stage3SVG />, color: "from-blue-900 to-black" },
    ];
        ...
        <div className="space-y-5">
            <div className="flex items-center justify-center gap-2">
                {[1, 2, 3].map((s) => (
                    <React.Fragment key={s}>
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                            completedStages.includes(s) ? "bg-green-500 text-white"
                                : currentStage === s ? "bg-[#f7623b] text-white ring-4 ring-orange-300 ring-opacity-40"
                                    : "bg-gray-700 text-gray-400"
                        }`}>
                            {completedStages.includes(s) ? <CheckCircle className="w-5 h-5" /> : s}
                        </div>
            ...
            <div className={`bg-gradient-to-br ${stageInfo.color} rounded-2xl p-5 border border-gray-700`}>
                ...
                <LivenessCamera key={currentStage} stage={currentStage} onStageComplete={handleStageComplete} />
            </div>
        </div>
    );
};
...
export default function GuarantorFormPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true);
                // ✅ Single call — /form/{token} returns all needed data
                const response = await axios.get(`${BASE_URL}/api/guarantor/form/${token}`);
    ...
    const handleSelfieComplete = (blob) => {
        ...
        setSelfieComplete(true);
        Swal.fire({
            icon: "success", title: "Liveness Verified!",
            text: "Your selfie with ID has been captured successfully.",
            confirmButtonColor: "#f7623b", timer: 2500, timerProgressBar: true,
        });
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${BASE_URL}/api/guarantor/submit/${token}`, payload);
    ...
    // ✅ Field names mapped directly from /form/{token} response
    ...
    const productPrice = customerData.price
        ? `₦${Number(customerData.price).toLocaleString()}`
        : null;
            ...
            <div className="max-w-2xl mx-auto">
                <div className="bg-black rounded-2xl shadow-2xl p-5 sm:p-8 space-y-6">
                    ...
                    <InfoCard title="Product Details" icon={<Package className="w-5 h-5 text-[#f7623b]" />}>
                        <InfoGrid items={[
                            { label: "Product Name", value: customerData.productName },
                            { label: "Brand",        value: customerData.productBrand },
                            { label: "Size",         value: customerData.productSize },
                            { label: "Price",        value: productPrice, highlight: true },
                        ]} />
                    </InfoCard>
                    ...
                    <InfoCard title="Customer Information" icon={<User className="w-5 h-5 text-[#f7623b]" />}>
                        <InfoGrid items={[
                            ...
                            { label: "Customer BVN",   value: customerData.customerBvn },
                            { label: "Customer Phone", value: customerData.customerPhoneNumber },
                            { label: "Customer Email", value: customerData.customerEmail },
                        ]} />
                    </InfoCard>
                    ...
                    <InfoCard title="Payment Plan" icon={<CreditCard className="w-5 h-5 text-[#f7623b]" />}>
                        <InfoGrid items={[
                            { label: "Plan Type",            value: customerData.customerPlan },
                            { label: "Duration",             value: customerData.installmentDuration ? `${customerData.installmentDuration} Months` : null },
                            { label: "Your Email (Guarantor)", value: customerData.guarantorEmail, full: true },
                        ]} />
                    </InfoCard>
