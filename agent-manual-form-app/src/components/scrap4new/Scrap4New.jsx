...
//                 </p>
//
//                 {/* Mandate badge — shown when coming from AgentEntryForm with verified account */}
//                 {incoming.mandateReference && (
...
//                                 <CheckCircle size={14} />
//                                 Account verified — {[incoming.firstName, incoming.middleName, incoming.lastName].filter(Boolean).join(' ')}
//                             </p>
//                         </div>
...
//                                     <p className="text-sm text-white font-mono break-all">{incoming.mandateReference}</p>
//                                 </div>
//                                 <span className="px-3 py-1 text-white text-xs font-semibold rounded-full whitespace-nowrap" style={{ backgroundColor: ACCENT }}>
//                                     Verified
//                                 </span>
//                             </div>
//                             {(incoming.bankName || incoming.accountNumber) && (
//                                 <div className="mt-2 pt-2 border-t border-gray-700 grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs">
//                                     {incoming.bankName && (
//                                         <div><span className="text-gray-400">Bank: </span><span className="text-white">{incoming.bankName}</span></div>
//                                     )}
//                                     {incoming.accountNumber && (
//                                         <div><span className="text-gray-400">Account: </span><span className="text-white">{incoming.accountNumber}</span></div>
//                                     )}
//                                 </div>
//                             )}
//                         </div>
...
const SCRAP_API = 'https://scrap4new-backend-production.up.railway.app/api/scrap-forms';
const ACCENT    = '#f7623b';
...
function Scrap4New() {
    ...
    const handleSubmit = async () => {
        ...
        fd.append('selected_issues',        form.selectedIssues.join(', '));
        fd.append('grand_total',            grandTotal.toFixed(2));
        fd.append('mandate_reference',      incoming.mandateReference  || '');
        fd.append('bank_name',              incoming.bankName          || '');
        fd.append('account_number',         incoming.accountNumber     || '');
